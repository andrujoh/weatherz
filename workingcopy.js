...
app.post('/', (req, res) => {
  ...
  request(url, (error, response, body) => {
    ...
    if (error || weather.main == undefined) {
      ...
      //Only add to database if unique, and if data changed since last time
        ...
        Weather.find({'city': city}).exec(function(err, result){
          ...
          if(result !== city.temp){
            Weather.update({city: city}, { $set: { temp: temp } }, function(err, updateCity){
              //req.flash('info', `Oppdaterte ${city}`);
              res.redirect('/');
              // res.render('index', {records: null, error: null, success: `Oppdaterte ${city}`});
            });
          }
        });
      // } else {
